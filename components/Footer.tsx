...
import { AdinkraSymbol } from "@/components/GhanaSymbols";
          ...
          <div className="col-span-2">
            ...
            <div className="mt-5 grid max-w-md grid-cols-4 gap-2">
              {[
                ...
                <div key={name} className="rounded-2xl border border-white/10 bg-white/5 p-3 text-center">
                  <AdinkraSymbol name={name as "gye-nyame" | "sankofa" | "dwennimmen" | "fawohodie"} className="mx-auto h-7 w-7 text-[#F2B84B]" />
                  <p className="mt-1 text-[10px] font-black uppercase tracking-[0.12em] text-white/45">{label}</p>
                </div>
              ))}
            </div>
